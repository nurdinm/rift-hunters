...
io.on("connection",(socket:GameSocket)=>{
 ...
 socket.on("hand:togglePause",d=>{if(!roomPayload(d))return;const r=getRoom(d.roomCode);if(!r||!validDisplay(r,d.token)||!(r.state.phase==="playing"||r.state.phase==="paused"))return;if(r.state.phase==="playing"){r.state.phase="paused";r.state.message="Paused by gesture";r.pausedAt=Date.now()}else{shiftTargetExpiry(r.state,Date.now()-(r.pausedAt??Date.now()));r.pausedAt=undefined;r.state.message=undefined;r.state.phase="playing"}broadcast(r)});
