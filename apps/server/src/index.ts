...
const begin=(r:Room)=>{stop(r);r.state={...initialState(r.state.code),controlMode:r.state.controlMode,players:r.state.players,phase:"countdown"};log("info","game.started",{roomCode:r.state.code});r.pausedAt=undefined;r.timer=setInterval(()=>{tickRoom(r);broadcast(r)},1000)};
...
io.on("connection",(socket:GameSocket)=>{
 ...
 socket.on("control:set",d=>{if(!roomPayload(d)||!(d.mode==="phone"||d.mode==="hand"))return;const r=getRoom(d.roomCode);if(!r||!validDisplay(r,d.token)||r.state.phase!=="lobby")return;r.state.controlMode=d.mode;if(d.mode==="hand")r.state.players={1:true,2:true};else for(const p of [1,2]as PlayerId[])r.state.players[p]=Boolean(r.playerSockets[p]);broadcast(r)});
