...
export type GameMode = "coop" | "solo";
...
export interface ClientToServerEvents {
 ...
 "game:start": (d:{roomCode:string;token:string})=>void; "game:pause": (d:{roomCode:string;token:string})=>void; "game:restart": (d:{roomCode:string;token:string})=>void; "game:mode": (d:{roomCode:string;token:string;mode:GameMode})=>void; "tutorial:set": (d:{roomCode:string;token:string;step:number})=>void; "room:close": (d:{roomCode:string;token:string})=>void;
  "hand:ability": (d:{roomCode:string;token:string;playerId:PlayerId})=>void; "hand:super": (d:{roomCode:string;token:string})=>void; "hand:togglePause": (d:{roomCode:string;token:string})=>void;
...
export interface AbilityEvent { playerId: PlayerId; kind: "ability" | "super" }
export interface ServerToClientEvents { "room:state":(s:PublicRoom)=>void; "shot:result":(r:ShotEvent)=>void; "shot:ability":(r:AbilityEvent)=>void; "room:error":(m:string)=>void }
